...
// 1. NOVA FUNÇÃO: Captura quais toggles de funcionários estão abertos
...
// 2. MODIFICAÇÃO: renderizarCorrecao agora aceita o estado dos toggles abertos
function renderizarCorrecao(pontoPorFuncionarioData, openToggles = {}) {
  ...
  for (const setor of Object.keys(setores)) {
    ...
    for (const func of setores[setor]) {
      ...
      const diasComRegistros = Object.keys(pontoDias).map(d => parseInt(d, 10));
      const ultimaData = diasComRegistros.length > 0 ? Math.max(...diasComRegistros) : 0;
      const diasParaMostrar = diasEscala.filter(dia => dia <= ultimaData);
      ...
      // 3. ADIÇÃO: Adiciona o atributo de dados para identificar o funcionário
      ...
      // 3. MODIFICAÇÃO: Verifica se deve começar aberto
      ...
      // 3. MODIFICAÇÃO: Esconde/mostra a tabela com base no estado salvo
        ...
        for (let idx = 0; idx < registrosDia.length; idx++) {
          ...
          ((remIndex) => {
            ...
            btnExcluir.addEventListener('click', () => {
              // 4. MODIFICAÇÃO: Captura o estado antes de re-renderizar
              const openState = getOpenToggles();

              registrosDia.splice(remIndex, 1);
              // 4. MODIFICAÇÃO: Passa o estado para manter a tela aberta
